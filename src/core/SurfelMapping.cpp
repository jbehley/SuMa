...
void SurfelMapping::setParameters(const rv::ParameterList& const_params) {
  ...
  if (fallbackMode_) {
    ...
    fallback_params.insert(FloatParameter("icp-max-distance", float(params["fallback-max-distance"])));
    fallback_params.insert(FloatParameter("icp-max-angle", float(params["fallback-max-angle"])));
  ...
  float p_unstable = 0.1;
  log_unstable_ = std::log(p_unstable / (1.0 - p_unstable));
...
void SurfelMapping::processScan(const rv::Laserscan& scan) {
  ...
  double completeTime = Stopwatch::toc();
...
void SurfelMapping::preprocess() {
  ...
  //  intermediateFrame_->copy(*currentFrame_);
...
void SurfelMapping::updatePose() {
  ...
  // use sub-sampled evaluation to check simply multiple initializations

//  if (timestamp_ < 3) {
//    objective_->setParameter(FloatParameter("distance_thresh", 3.0f));
//  } else if (timestamp_ == 4) {
//    objective_->setParameter(FloatParameter("distance_thresh", 0.5f));
//  }
...
void SurfelMapping::checkLoopClosure() {
  ...
  if (unverifiedLoopClosures_.size() > 0 || alreadyVerifiedLoopClosure_) {
    //    std::cout << "[info] Verifying loop closure candidate." << std::endl;
    ...
    statistics_["increment_difference"] = increment_difference;
    //    std::cout << increment_difference << std::endl;
  ...
  if (!alreadyVerifiedLoopClosure_ && int32_t(unverifiedLoopClosures_.size()) >= (loopMinNumberVerifications_ + 1)) {
    //    std::cout << "[info] Verified last loop closure." << std::endl;
  ...
  //  if (verifiedLoopClosures_.size() > 6 || timeWithoutLoopClosure_ > 3)
  {
    for (const auto& candidate : verifiedLoopClosures_) {
      ...
      //      std::cout << "Adding constraints " << candidate.from << " -> " << candidate.to << std::endl;
      if (lastFrom != from) {
        lastFrom = from;
        loopCount_ += 1;
      }
  ...
  if (timeWithoutLoopClosure_ > 3) {
    //    std::cout << "[info] Searching for loop closure candidate." << std::endl;
    ...
    for (uint32_t c = 0; c < candidateTimestamps.size(); ++c) {
      ...
      //      std::cout << "Rendering: " << Stopwatch::toc() * 1000 << " ms" << std::endl;
      ...
      for (uint32_t i = 0; i < initializations.size(); ++i) {
        ...
        //        std::cout << "Optimization: " << optimizationTime << " ms ("
        //                  << 100 * optimizationTime / (optimizationTime + renderingTime) << "%)";
        //        std::cout << ", Rendering: " << renderingTime << " ms ("
        //                  << 100 * renderingTime / (optimizationTime + renderingTime) << "%)" << std::endl;
      }
  ...
  //  std::cout << "Current  : R = " << result_new_.residual
  //            << ", r(i) = " << result_new_.inlier_residual / result_new_.inlier
  //            << ", V = " << result_new_.valid / float(result_new_.valid + result_new_.invalid) << ", "
  //            << "O = " << result_new_.outlier / float(result_new_.outlier + result_new_.inlier) << std::endl;
  //
  //  std::cout << "Candidate: R = " << result_old_.residual
  //            << ", r(i) = " << result_old_.inlier_residual / result_old_.inlier
  //            << ", V = " << result_old_.valid / float(result_old_.valid + result_old_.invalid) << ", "
  //            << "O = " << result_old_.outlier / float(result_old_.outlier + result_old_.inlier) << std::endl;
...
bool SurfelMapping::optimizeAsync() {
  //  std::cout << ">>> Called optimized asynchronously at t = " << timestamp_ << "..." << std::endl;
