...
Matrix::Matrix(const int32_t m_, const int32_t n_, const FLOAT *val_) {
  ...
  for (int32_t i = 0; i < m_; i++)
    for (int32_t j = 0; j < n_; j++) val[i][j] = val_[k++];
}

Matrix::Matrix(const Matrix &M) {
  allocateMemory(M.m, M.n);
  for (int32_t i = 0; i < M.m; i++) memcpy(val[i], M.val[i], M.n * sizeof(FLOAT));
}
...
void Matrix::getData(FLOAT *val_, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  ...
  for (int32_t i = i1; i <= i2; i++)
    for (int32_t j = j1; j <= j2; j++) val_[k++] = val[i][j];
}
...
Matrix Matrix::getMat(int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  ...
  for (int32_t i = 0; i < M.m; i++)
    for (int32_t j = 0; j < M.n; j++) M.val[i][j] = val[i1 + i][j1 + j];
  return M;
}
...
void Matrix::setDiag(FLOAT s, int32_t i1, int32_t i2) {
  ...
  for (int32_t i = i1; i <= i2; i++) val[i][i] = s;
}
...
Matrix Matrix::extractCols(vector<int> idx) {
  ...
  for (int32_t j = 0; j < M.n; j++)
    if (idx[j] < n)
      for (int32_t i = 0; i < m; i++) M.val[i][j] = val[i][idx[j]];
  return M;
}

Matrix Matrix::eye(const int32_t m) {
  Matrix M(m, m);
  for (int32_t i = 0; i < m; i++) M.val[i][i] = 1;
  return M;
}

void Matrix::eye() {
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) val[i][j] = 0;
  for (int32_t i = 0; i < min(m, n); i++) val[i][i] = 1;
}

Matrix Matrix::diag(const Matrix &M) {
  if (M.m > 1 && M.n == 1) {
    Matrix D(M.m, M.m);
    for (int32_t i = 0; i < M.m; i++) D.val[i][i] = M.val[i][0];
    return D;
  } else if (M.m == 1 && M.n > 1) {
    Matrix D(M.n, M.n);
    for (int32_t i = 0; i < M.n; i++) D.val[i][i] = M.val[0][i];
    return D;
  }
...
Matrix Matrix::reshape(const Matrix &M, int32_t m_, int32_t n_) {
  if (M.m * M.n != m_ * n_) {
    cerr << "ERROR: Trying to reshape a matrix of size (" << M.m << "x" << M.n << ") to size (" << m_ << "x" << n_
         << ")" << endl;
    exit(0);
  }
  Matrix M2(m_, n_);
...
Matrix Matrix::operator+(const Matrix &M) {
  ...
  const Matrix &B = M;
  if (A.m != B.m || A.n != B.n) {
    cerr << "ERROR: Trying to add matrices of size (" << A.m << "x" << A.n << ") and (" << B.m << "x" << B.n << ")"
         << endl;
    exit(0);
  }
  Matrix C(A.m, A.n);
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) C.val[i][j] = A.val[i][j] + B.val[i][j];
  return C;
}
...
Matrix Matrix::operator-(const Matrix &M) {
  ...
  const Matrix &B = M;
  if (A.m != B.m || A.n != B.n) {
    cerr << "ERROR: Trying to subtract matrices of size (" << A.m << "x" << A.n << ") and (" << B.m << "x" << B.n << ")"
         << endl;
    exit(0);
  }
  Matrix C(A.m, A.n);
...
Matrix Matrix::operator*(const Matrix &M) {
  ...
  const Matrix &B = M;
  if (A.n != B.m) {
    cerr << "ERROR: Trying to multiply matrices of size (" << A.m << "x" << A.n << ") and (" << B.m << "x" << B.n << ")"
         << endl;
    exit(0);
  }
  Matrix C(A.m, B.n);
  for (int32_t i = 0; i < A.m; i++)
    for (int32_t j = 0; j < B.n; j++)
      for (int32_t k = 0; k < A.n; k++) C.val[i][j] += A.val[i][k] * B.val[k][j];
  return C;
}
...
Matrix Matrix::operator*(const FLOAT &s) {
  Matrix C(m, n);
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) C.val[i][j] = val[i][j] * s;
  return C;
}
  ...
  if (A.m == B.m && A.n == B.n) {
    ...
    for (int32_t i = 0; i < A.m; i++)
      for (int32_t j = 0; j < A.n; j++)
        if (B.val[i][j] != 0) C.val[i][j] = A.val[i][j] / B.val[i][j];
    return C;
    ...
    for (int32_t i = 0; i < A.m; i++)
      for (int32_t j = 0; j < A.n; j++)
        if (B.val[i][0] != 0) C.val[i][j] = A.val[i][j] / B.val[i][0];
    return C;
    ...
    for (int32_t i = 0; i < A.m; i++)
      for (int32_t j = 0; j < A.n; j++)
        if (B.val[0][j] != 0) C.val[i][j] = A.val[i][j] / B.val[0][j];
    return C;

  } else {
    cerr << "ERROR: Trying to divide matrices of size (" << A.m << "x" << A.n << ") and (" << B.m << "x" << B.n << ")"
         << endl;
    exit(0);
  }
}
...
Matrix Matrix::operator-() {
  Matrix C(m, n);
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) C.val[i][j] = -val[i][j];
  return C;
}
...
Matrix Matrix::operator~() {
  Matrix C(n, m);
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) C.val[j][i] = val[i][j];
  return C;
}
...
FLOAT Matrix::l2norm() {
  FLOAT norm = 0;
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) norm += val[i][j] * val[i][j];
  return sqrt(norm);
}
...
FLOAT Matrix::mean() {
  FLOAT mean = 0;
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++) mean += val[i][j];
...
FLOAT Matrix::det() {
  ...
  FLOAT d;
  A.lu(idx, d);
  for (int32_t i = 0; i < m; i++) d *= A.val[i][i];
  free(idx);
...
bool Matrix::solve(const Matrix &M, FLOAT eps) {
  ...
  if (A.m != A.n || A.m != B.m || A.m < 1 || B.n < 1) {
    cerr << "ERROR: Trying to eliminate matrices of size (" << A.m << "x" << A.n << ") and (" << B.m << "x" << B.n
         << ")" << endl;
    exit(0);
  }
...
void Matrix::allocateMemory(const int32_t m_, const int32_t n_) {
  ...
  for (int32_t i = 1; i < m; i++) val[i] = val[i - 1] + n;
}
